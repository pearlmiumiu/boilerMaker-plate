...
###sequelize
  ...
  read database url from enviroment(optional)
  define models and establish associations
  sync your database

###authentication
  install   
  //If we want authentication in our app, then we'll need sessions as well. npm install --save express-session passport.
  session middleware
  ...
  log out
  get me
  plug into the express pipeline

###auth2
  install   //npm install --save passport passport-google-oauth
  configure provider app
  protect credentials
  add google id to user model
  redirect when user petitons to use oauth
  configure callback
  configure strategy
  plug into your express pipline
###testing
  install
  test script
